...
Client.prototype.update = function(data,options,callback){
   ...
   if (!options.extract)
     ...
     options.stream = data;
   var url = (options.update || 'update/json');
   if (url.indexOf('?') == -1)
     url = url + '?commit='+ (options.autoCommit || this.autoCommit) +'&wt=json';
   options.fullPath = [options.path,options.core || options.core, url]
                              .filter(function(element){
                                 if(element) return true;
                                 return false;    
                              })
